...
import { DEMO_RAILML } from '../src/data/demo-layout';
...
describe('End-to-end pipeline', () => {
  it('parses demo railML without errors', () => {
    ...
    expect(infra.tracks.length).toBeGreaterThan(0);
    expect(infra.signals.length).toBeGreaterThan(0);
    expect(infra.bufferStops.length).toBeGreaterThan(0);
    expect(infra.stations.length).toBeGreaterThan(0);
  ...
  it('builds topology without errors', () => {
    ...
    expect(bufferNodes.length).toBeGreaterThanOrEqual(2);

    // Should have switch nodes (from merged colocated nodes)
  ...
  it('runs train dynamics simulation without errors', () => {
    ...
    // Run 500 simulation steps (should cover acceleration + braking)
    for (let i = 0; i < 500; i++) {
      dynamics.update(0.1);
  ...
  it('runs full simulation to completion', () => {
    ...
    expect(finalState.totalDistance).toBeGreaterThan(100);
  });
});
